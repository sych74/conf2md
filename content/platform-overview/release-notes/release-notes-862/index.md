...
aliases: "/release-notes-862/"
seoindex: "index, follow"
seotitle: "Release Notes 8.6.2"
seokeywords: "paas, paas release notes, paas version, 8.6.2 release"
seodesc: "In this document, you will find all of the new features, enhancements and visible changes included to the Virtuozzo PaaS 8.6.2 release."
menu:
    docs:
        title: "Release Notes 8.6.2"
        url: "/release-notes-862/"
        weight: 24
        parent: "release-notes"
        identifier: "release-notes-862.md"
---

<a id="back"></a>

# Virtuozzo Application Platform 8.6.2

*This document is preliminary and subject to change.*

In this document, you will find all of the new features, enhancements and visible changes included to the **Virtuozzo PaaS 8.6.2** release.
...
Below, you can find a list of all changes to the public API in the 8.6.2 platform version (compared to the preceding [8.4](/release-notes-84/#api-changes) ones):

- Added a new ***relatedEnvName*** parameter to point to the environment associated with the firewall rule (for the **AddRules**, **EditRule**, and **AddRule** methods)
...
Below, you can find the fixes that were implemented in the Virtuozzo Application Platform 8.6.2 release and also integrated into previous platform versions by means of the appropriate patches.

{{%table3 title="Virtuozzo Application Platform 8.6.2"%}}
**#**|**Compatible from**|**Description**
---|:---:|---
JE-69066|any|Duplicated cron tasks for the *Let’s Encrypt SSL* add-on in environments cloned by the collaborator
JE-70084|any|The ports conflict between the *Let's Encrypt SSL* and *BitNinja* add-ons
JE-70832|any|An error occurs when deploying an application to the *WildFly* cluster
JE-71137|any|The Tinyproxy daemon for the *Let’s Encrypt SSL* add-on does not listen to the IPv6 address on the *AlmaLinux-based* installations
JE-62980|5.0.5|For the *Docker CE* container, move the Portainer manager admin port from *443* to *4848*
JE-65840|5.4|An error occurs when deploying an application to the *.NET* container
JE-71280|8.3|Incorrect regular expression for the "*icmp host-prohibited*" rule in the nftables on the *Node.js* and *Go* nodes
{{%/table3%}}
...
In the table below, you can see the list of bug fixes applied to the platform starting from Virtuozzo Application Platform 8.6.2 release:

{{%table3 title="Virtuozzo Application Platform 8.6.2"%}}
**#**|**Affected Versions**|**Description**
---|:---:|---
JE-70957|8.6|An error occurs when adding a firewall rule for the shared environment while adding another shared environment as a source
JE-71100|8.6|Add-ons with the scheduler stop working after a collaborator (who installed it) is removed from the collaboration
JE-71278|8.3|Incorrect regular expression for the "*icmp host-prohibited*" rule in the nftables
JE-71282|8.4.3|The *Maven* build node cannot execute the "*Build and Deploy*" action if auto-deploy is enabled
JE-71387|8.6|The tasks configured by the collaboration member on the owner’s environments are destroyed/deactivated alongside the member’s account
{{%/table3%}}
