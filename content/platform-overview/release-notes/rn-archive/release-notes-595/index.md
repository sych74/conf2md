---
draft: false
title: "Release Notes 5.9.5"
aliases: ["/release-notes-595/", "/release-notes-594/"]
seoindex: "index, follow"
seotitle: "Release Notes 5.9.5"
seokeywords: "paas, paas release notes, paas version, 5.9.5 release"
seodesc: "In this document, you will find all of the new features, enhancements and visible changes included to the platform 5.9.5 release."
---

<a id="back"></a>

# Virtuozzo Application Platform 5.9.5

*This document is preliminary and subject to change.*

In this document, you will find all of the new features, enhancements and visible changes included to the **PaaS 5.9.5** release.
...
Below, you can find the fixes that were implemented in the PaaS 5.9.5 release and also integrated into previous platform versions by means of the appropriate patches.

{{%table3 title="PaaS 5.9.5"%}}
**#**|**Compatible from**|**Description**
---|:---:|---
JE-55205|3.3|The fallback mechanism of the *Let's Encrypt* add-on should be performed only for non-custom domains
JE-55663|3.3|The *Node-RED Dev* package cannot be installed due to the source repository rename
JE-50418|5.0.5|An error occurs when running the *httpd* command under the non-root user on the *Apache* stacks
JE-55235|5.4|Applications deployed to the non-ROOT folder on the *.NET Core* stack don't work
JE-55333|5.4|The *gluster-recovery.service* fails to start on the GlusterFS storage cluster during creation
JE-55375|5.8.1|An error occurs when installing the *Jira* application from the Marketplace
JE-55244|5.9.2|*LiteSpeed-based* instances should be reloaded (instead of restart) if the number of workers was not changed
{{%/table3%}}
...
In the table below, you can see the list of bug fixes applied to the platform starting from PaaS 5.9.5 release:

{{%bug-fixes title="PaaS 5.9.5"%}}
**#**|**Description**
---|---
JE-54515|An environment cannot be created via API/CLI request if the *cloudlets* parameter is not defined
JE-55252|When creating an environment, the *StackOverflowError* is returned instead of an actual error
JE-55268|An error occurs when cloning *Node.js* container with NFS mount
JE-55406|An environment cannot be created via API/CLI request if the *region* parameter is not defined
JE-55409|The *globals* variables cannot be used in the *mixins* section of the CS manifest
JE-55556|Incorrect caption for the *Billing History* tab in the dashboard
JE-55560|An error occurs when creating complex environments (like *WordPress Cluster*)
JE-55667|An error occurs upon clicking the *Reset Password* button in email
{{%/bug-fixes%}}
