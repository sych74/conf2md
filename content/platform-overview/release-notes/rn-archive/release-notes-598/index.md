---
draft: false
title: "Release Notes 5.9.8"
aliases: "/release-notes-598/"
seoindex: "index, follow"
seotitle: "Release Notes 5.9.8"
seokeywords: "paas, paas release notes, paas version, 5.9.8 release"
seodesc: "In this document, you will find all of the new features, enhancements and visible changes included to the platform 5.9.8 release."
---

<a id="back"></a>

# Virtuozzo Application Platform 5.9.8

*This document is preliminary and subject to change.*

In this document, you will find all of the new features, enhancements and visible changes included to the **PaaS 5.9.8** release.

{{%rn-changed%}}
{{%rn-item url="#beta-label-removal-for-storage-auto-clustering"%}}
## Beta Label Removal for Storage Auto-Clustering
ARemoved the *beta* label for the Shared Storage auto-clustering
{{%/rn-item%}}
...
The [Shared Storage auto-clustering](/shared-storage-container/#shared-storage-auto-cluster) feature has successfully passed its beta period with positive feedback and without any major issues. So, starting with the PaaS 5.9.8 release, the *beta* UI label next to the related functionality is removed. The changes concern not only topology wizard, but JPS packages as well (e.g. WordPress Cluster).

[More info](/shared-storage-container/#shared-storage-auto-cluster)

{{%right%}}[Back to the top](#back){{%/right%}}

## API Changes

Below, you can find a list of all changes to the public API in the 5.9.8 platform version (compared to the preceding [5.9.3](/release-notes-593/#api-changes) ones):

- the *password* parameter became optional (depends on the platform configs) for the [token related API](https://docs.jelastic.com/api/#!/api/users.Authentication) - ***CreateToken***, ***DeleteTokens***, ***RegenerateToken***, ***EditToken***
- added the *passphrase* alias to the *password* parameter for the ***[DeleteEnv](https://docs.jelastic.com/api/#!/api/environment.Control-method-DeleteEnv)*** method from the **control** service
- the ***AttachAddon*** and ***DetachAddon*** methods from the **CloudScripting** service were removed
...
Below, you can find the fixes that were implemented in the PaaS 5.9.8 release and also integrated into previous platform versions by means of the appropriate patches.

{{%table3 title="PaaS 5.9.8"%}}
**#**|**Compatible from**|**Description**
---|:---:|---
JE-56377|3.3|Validation should be added to the *Time Zone* field in the *Env Start/Stop Scheduler* package at Marketplace
JE-56601|3.3|The *osTicket* package from Marketplace does not work after installation 
JE-56604|3.3|The *Node-RED Dev* package from Marketplace does not work after installation 
JE-56734|3.3|Incorrect configurations in the *php.ini* file for the *Drupal* package at Marketplace
JE-55281|5.4|The *GitLab* package at Marketplace should use the latest image version
{{%/table3%}}
...
In the table below, you can see the list of bug fixes applied to the platform starting from PaaS 5.9.8 release:

{{%table3 title="PaaS 5.9.8"%}}
**#**|**Affected Versions**|**Description**
---|:---:|---
JE-46177|-|Unhandled error when platform's Uploader cannot be connected during an archive removal
JE-54062|-|Container creation and redeployment fail for images with an empty *ImageId* value in manifest
JE-55561|-|Missing *EXPOSED* ports should be added to firewall rules during container redeploy
{{%/table3%}}
