---
draft: false
title: "Release Notes 6.0.6"
aliases: "/release-notes-606/"
seoindex: "index, follow"
seotitle: "Release Notes 6.0.6"
seokeywords: "paas, paas release notes, paas version, 6.0.6 release"
seodesc: "In this document, you will find all of the new features, enhancements and visible changes included to the platform 6.0.6 release."
---

<a id="back"></a>

# Virtuozzo Application Platform 6.0.6

*This document is preliminary and subject to change.*

In this document, you will find all of the new features, enhancements and visible changes included to the **PaaS 6.0.6** release.

{{%rn-changed%}}
{{%rn-item url="#kubernetes-cluster-domain-length-validation"%}}
## Kubernetes Cluster Domain Length Validation
Adjusted domain length validation for the *Kubernetes Cluster* package due to official requirements
{{%/rn-item%}}
...
The total length of the container hostname is limited to **64** characters due to the Linux specifics. The platform automatically validates this value during environment creation. However, for Kubernetes instances, the requirement is a bit lower - **63**. So, starting with the platform 6.0.6 release, such nuance is correctly considered during the ***Kubernetes Cluster*** package creation to avoid possible errors due to incorrect domain length.

![Kubernetes domain length](01-kubernetes-domain-length.png)
...
Below, you can find the fixes that were implemented in the PaaS 6.0.6 release and also integrated into previous platform versions by means of the appropriate patches.

{{%table3 title="PaaS 6.0.6"%}}
**#**|**Compatible from**|**Description**
---|:---:|---
JE-58049|3.3|Rate limit error for custom domains should be handled during the *Let's Encrypt* add-on installation
JE-58154|3.3|A test SSL certificate is provided for the *GitLab* application when reconfiguring the *Let's Encrypt* add-on to a new domain
JE-53826|5.0.5|Incorrect admin console link on the home page of the *WildFly cluster* if the environment name include non-English characters
JE-56791|5.0.5|Version-specific configs are missing on the *WildFly 20* and *21* application servers
JE-57152|5.0.5|An error occurs when redeploying the *Ruby* application server to a lower version
JE-57741|5.0.5|The *Node-RED Dev* application is inaccessible after deployment from the Marketplace
JE-58773|5.0.5|An error occurs when trying to clone an environment with a *Tomcat cluster*
JE-58804|5.0.5|Typo in the resource name in the *ssl.upstreams.inc* configuration file on the *NGINX load balancer*
JE-52157|5.3.2|An error occurs when switching to the *Runtime* tab in the *WildFly 17* and *18* admin panel
JE-58544|5.4|The "*jem vcs update*" error occurs on the Golang application server when deploying some applications
JE-55261|5.7|The administration panel for the *WildFly 21 cluster* is inaccessible via the DAS node on the *PaaS 5.7* platforms
JE-57639|5.7|Webhook of the *Git-Push-Deploy* add-on fails to trigger project updates
{{%/table3%}}
...
In the table below, you can see the list of bug fixes applied to the platform starting from PaaS 6.0.6 release:

{{%table3 title="PaaS 6.0.6"%}}
**#**|**Affected Versions**|**Description**
---|:---:|---
JE-58281|5.9.3|Iptables rules are not removed on the *Ubuntu VPS* after disabling the firewall via the dashboard
JE-58284|6.0.2|Duplicate iptables entries in firewall after the *Ubuntu VPS* redeploy
JE-58682|-|An error occurs during environment migration between regions
JE-58716|-|Custom firewall rules are added incorrectly on the *Ubuntu VPS* nodes
JE-58839|-|An error occurs when trying to access the container file manager via the dashboard
{{%/table3%}}
